/*
 * File:     prog3.1_histo_dist.c
 *
 * Purpose:  Use MPI to implement a program that creates a histogram
 *
 * Compile:  mpicc -g -Wall -o prog3.1_histo_dist prog3.1_histo_dist.c
 * Run:      mpiexec -n <comm_sz> ./prog3.1_histo_dist
 *
...
int Which_bin(float data, float bin_maxes[], int bin_count,
    float min_meas) {
    /* TODO */
    for (int i = 0; i < bin_count; i++) //since all the data in [min_meas, max_meas), no need to treat bin0 as special case
    {
        if (data < bin_maxes[i]) return i;
    }
    return -1;
}  /* Which_bin */
