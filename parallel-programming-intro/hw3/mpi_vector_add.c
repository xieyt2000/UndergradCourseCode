...
void Read_vector(double local_a[], int local_n, int n, char vec_name[],
    int my_rank, MPI_Comm comm, int send_counts[], int displs[]);
void Print_vector(double local_b[], int local_n, int n, char title[],
    int my_rank, MPI_Comm comm, int send_counts[], int displs[]);
void Parallel_vector_sum(double local_x[], double local_y[],
    double local_z[], int local_n);

//allocate and cal send_counts and sipls
void Calculate_counts(int** send_counts_pp, int**displs_pp,
                        int n, int comm_sz)
{
    int div = n / comm_sz, mod = n % comm_sz;
    *send_counts_pp = malloc(comm_sz * sizeof(int));
    *displs_pp = malloc(comm_sz * sizeof(int));
    for (int i = 0; i < comm_sz; i++)
    {
        if (i < mod)
        {
            (*send_counts_pp)[i] = div + 1;
            (*displs_pp)[i] = i * (div + 1);
        }
        else
        {
            (*send_counts_pp)[i] = div;
            (*displs_pp)[i] = i * div + mod;
        }
    }
}
...
int main(void) {
    ...
    MPI_Comm comm;
    int* send_counts = NULL, * displs = NULL; //for scatterv and gather v. only proc 0 need this.
...
#  endif
    ...
    if (my_rank == 0)
    {
        Calculate_counts(&send_counts, &displs, n, comm_sz);
#ifdef DEBUG
        for (int i = 0; i < comm_sz; i++)
        {
            printf("core %d: count%d disps%d\n", i, send_counts[i], displs[i]);
        }
#endif // DEBUG

    }
        
    Read_vector(local_x, local_n, n, "x", my_rank, comm, send_counts, displs);
    Print_vector(local_x, local_n, n, "x is", my_rank, comm, send_counts, displs);
    Read_vector(local_y, local_n, n, "y", my_rank, comm, send_counts, displs);
    Print_vector(local_y, local_n, n, "y is", my_rank, comm, send_counts, displs);


    Parallel_vector_sum(local_x, local_y, local_z, local_n);
    Print_vector(local_z, local_n, n, "The sum is", my_rank, comm, send_counts, displs);
...
void Read_n(
    ...
    Check_for_error(local_ok, fname,
        "n should be > 0", comm);
    int div = *n_p / comm_sz, mod = *n_p % comm_sz;
    if (my_rank < mod) *local_n_p = div + 1;
    else *local_n_p = div;
}  /* Read_n */
...
void Read_vector(
    ...
    if (my_rank == 0) {
        ...
        for (i = 0; i < n; i++)
            scanf("%lf", &a[i]);
        MPI_Scatterv(a, send_counts, displs, MPI_DOUBLE, local_a, local_n, MPI_DOUBLE, 0,
            comm);
        free(a);
    }
    else {
...
void Print_vector(
    ...
    if (my_rank == 0) {
        ...
        Check_for_error(local_ok, fname, "Can't allocate temporary vector",
            comm);
        MPI_Gatherv(local_b, local_n, MPI_DOUBLE, b, recv_counts, displs, MPI_DOUBLE,
            0, comm);
        printf("%s\n", title);
        for (i = 0; i < n; i++)
            printf("%f ", b[i]);
