...
# TODO: 1. Change N_Fourier to 2, 4, 8, 16, 32, 64, 128,
#  get visualization results with different number of Fourier Series
N_Fourier = 64
...
def fourier_coefficient(n):
    # int_fun: function to integrate
    if n == 0:  # a0
        def int_fun(t):
            return function(t) / (2 * pi)
    elif n % 2 == 0:  # am
        m = n / 2

        def int_fun(t):
            return function(t) / pi * (math.cos(m * t))
    else:  # bm
        m = (n + 1) / 2

        def int_fun(t):
            return function(t) / pi * (math.sin(m * t))
    return quad(int_fun, 0, 2 * pi)[0]
...
def semi_circle_wave(t):
    if 0 <= t <= 2 * pi:
        return math.sqrt(pi ** 2 - (t - pi) ** 2)
    elif t >= 2 * pi:
        return semi_circle_wave(t - 2)
    return semi_circle_wave(t + 2)
...
def visualize():
    ...
    imageio.mimsave('{}-{}.mp4'.format(signal_name, N_Fourier), images)


if __name__ == "__main__":
    for n_fou in [2, 4, 8, 16, 32, 64, 128]:
        N_Fourier = n_fou
        visualize()
        print(f'{N_Fourier} done.')
