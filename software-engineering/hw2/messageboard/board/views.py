...
def message(request):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            # PS: 请求数据体应该为{"title": "something", "content": "something"} ，请确保title和content字段存在，并且title和content均有最大长度限制
            # PS: 检测方式可以参考user，使用Django提供的full_clean()方法进行检测
            # --------------------------------------------------------------------------------------------
            post_message = Message(user=user, title=request_body["title"], content=request_body["content"])
            post_message.full_clean()
            post_message.save()
        except json.JSONDecodeError as e:
            return gen_response(400, f"JSON Decoder Error of post body {e}")
        except ValidationError as e:
            return gen_response(400, f"Validation Error of message {e}")
        except KeyError as e:
            return gen_response(400, f"Key Error of post body {e}")
