...
public:
	...
	ostream & print(ostream & out, bool isCOUT) const;
	~PrintableTable();

	ostream & orderprint(ostream & out, string orderbywhat) const;
	ostream & countprint(ostream & out, string countwhat) const;
	ostream & groupprint(ostream & out, bool hasCount, string countwhat, vector<string> groupbywhat, bool hasOrderBy, string orderbywhat) const;
...
private:
	...
	string errMsg;
public:
	vector<Record> getData() const { return data; }
	const string name;
	const vector<Attribute> attrs;
	AttributeType askType(const string & _attrName) const;
