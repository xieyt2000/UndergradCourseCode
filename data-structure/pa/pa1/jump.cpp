...
private:
	int data[MAX_POINTS];	//ascending order by minTime
	int left = 0;	//data[left]:min
	int right = 0;	//data[right-1]:max

public:
	unsigned long long minTime[MAX_POINTS];	//min time fron poiont i to n. minTime[n]==0 and minTime[1]==result
...
int t[MAX_POINTS], l[MAX_POINTS], r[MAX_POINTS];	//same to the text
MonoQueue queue;
int n;
