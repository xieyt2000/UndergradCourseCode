...
class Trainer(object):
    ...
    def loss_fn(self, target, recon, mu, log_var):
        ...
        *   Returns:
        ...
        '''
        # TODO START
        # use sum / batch_size to get average (mean might not be ok)
        recon_loss = nn.BCELoss(reduction='sum')(recon, target) / target.size()[0]
        kl_loss_batch = 0.5 * torch.sum(mu ** 2 + log_var.exp() - log_var - 1, dim=-1)
        kl_loss = kl_loss_batch.mean()
