...
class Trainer(object):
    ...
    def train_step(self, real_imgs, fake_imgs, BCE_criterion):
        ...
        *   Returns:
        ...
        loss_D_real = BCE_criterion(D_real, torch.ones(D_real.size(), device=self._device))
        D_x = D_real.mean()
        ...
        # TODO START
        D_fake = self._netD(fake_imgs.detach())
        loss_D_fake = BCE_criterion(D_fake, torch.zeros(D_fake.size(), device=self._device))
        D_G_z1 = D_fake.mean()
        ...
        loss_G = BCE_criterion(D_fake, torch.ones(D_fake.size(), device=self._device))
        D_G_z2 = D_fake.mean()
        loss_G.backward()
        # TODO END
