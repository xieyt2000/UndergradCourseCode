...
def train_net(model, loss, config, inputs, labels, batch_size, disp_freq):
    ...
    # record loss and accuracy for tensorboard
    ...
    for input, label in data_iterator(inputs, labels, batch_size):
        ...
        if iter_counter % disp_freq == 0:
            loss_rec.append(np.mean(loss_list))
            accuracy_rec.append(np.mean(acc_list))
            msg = '  Training iter %d, batch loss %.4f, batch acc %.4f' % (iter_counter, loss_rec[-1], accuracy_rec[-1])
            loss_list = []
            acc_list = []
            LOG_INFO(msg)
    return loss_rec, accuracy_rec
...
def test_net(model, loss, inputs, labels, batch_size):
    ...
    # record loss and accuracy for tensorboard
    loss_rec, accuracy_rec = np.mean(loss_list), np.mean(acc_list)
    msg = '    Testing, total mean loss %.5f, total acc %.5f' % (loss_rec, accuracy_rec)
    LOG_INFO(msg)
    return loss_rec, accuracy_rec
